...
export class AuthService {
    ...
    private handleErrors(a) {
        switch (a.type) {
            case AppActions.SESSION_DONE:
                if (a.payload instanceof AuthError) {
                    this.appTasks.refreshTokenTask();
                } else if (a.payload instanceof AccountError) {
                    this.goLoginPage();
                } else if (a.payload instanceof NetworkError || a.payload instanceof HttpError) {
                    this.retryLater();
                } else {
                    this.retryLater();
                }
    ...
    private goLoginPage() {
        if (this.account && this.account.isOAuth) {
