...
import { WebSqlDatabase } from './database/web-sql-database';
import { Device } from '../../models/device';
import { Group } from '../../models/group';

export class WebSqlCore implements StorageCore {

    private _db: WebSqlDatabase;

    constructor(
    ) {
        this._db = new WebSqlDatabase();
    }
    ...
    private updateDevice(device): Promise<any> {
        ...
        let queries = [];
        queries.push({
            sql: 'UPDATE device_model SET connected=?, device_state=?, profile_esh_class=?, profile_esh_esh_version=?, profile_esh_device_id=?, profile_esh_brand=?, profile_esh_model=?, profile_module_fw_version=?, profile_module_mac_address=?, profile_module_local_ip=?, profile_module_ssid=?, profile_cert=?, calendar=?, status=?, fields=?, users=? WHERE device_sn=?;',
            params: [connected, device_state, profile_esh_class, profile_esh_esh_version, profile_esh_device_id, profile_esh_brand, profile_esh_model, profile_module_fw_version, profile_module_mac_address, profile_module_local_ip, profile_module_ssid, profile_cert, calendar, status, fields, users, sn]
        });
    ...
    private makeDevice(dCursor): Device {
        return {
            ...
            profile: {
                ...
                cert: JSON.parse(dCursor.profile_cert || '{}'),
            },
            calendar: JSON.parse(dCursor.calendar || '[]'),
            status: JSON.parse(dCursor.status || '{}'),
            fields: JSON.parse(dCursor.fields || '[]'),
            users: JSON.parse(dCursor.users || '[]'),
            groups: [],
            role: dCursor.role,
            owner: dCursor.owner,
            properties: JSON.parse(dCursor.properties || '{}'),
        };
    }
    ...
    private makeGroup(gCursor): Group {
        return {
            ...
            devices: [],
            properties: JSON.parse(gCursor.properties || '{}'),
        };
    }
