...
@Component({
  selector: 'large-toggle-with-range',
  templateUrl: 'large-toggle-with-range.html'
})
export class LargeToggleWithRange extends UIComponentBase {
  ...
  constructor(
    ...
    super(ims, 'large-toggle-with-range', null);
  ...
  protected processLayout(model: ComponentModel, values: Array<ValueItem> | UIOptions, key: string, index: number, unitModel: ControlItemModel) {
    ...
    if (index === 0) {
      this.rangeState = this.rangeLogic.processLayout(values, key, unitModel);
    } else if (index === 1) {
