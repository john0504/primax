...
export abstract class UIComponentBase implements UIComponent {
    ...
    if (model.models && model.models.length > 0) {
      model.models.forEach((unitModel: ControlItemModel, index) => {
        if (unitModel.key) {
          ...
          this.processLayout(model, v, key, index, unitModel);
  ...
  protected abstract processLayout(model: ComponentModel, values: Array<ValueItem> | UIOptions, key: string, index: number, unitModel: ControlItemModel);
