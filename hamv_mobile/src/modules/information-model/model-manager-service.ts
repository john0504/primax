...
export class ModelManagerService {
  ...
  public load(): Promise<any> {
    ...
    zip(
      ...
      .subscribe(() => { }, () => { }, () => {
        Logger.log('Cloud - information model loaded');
  ...
  private listModelsFromCloud(): Observable<any> {
    return this.http.get(this.baseUrl + '/api:1/info-model')
  ...
  private loadFromCloud(familyName, cloudFile): Observable<any> {
    const url = this.baseUrl + '/api:1/info-model/' + familyName;
    const filename = cloudFile.timestamp + familyName;
  ...
  private loadFromJson(model: any = {}): any {
    if (!model.familyName) {
      return Logger.error('Information Model do not have familyName', model);
    }
